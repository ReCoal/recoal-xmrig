...
#   endif
    ...
    Log::i()->text(config->isColors() ? "\x1B[01;32m * \x1B[01;37mVERSIONS:     \x1B[01;36mReCoal-XMRig/%s\x1B[01;37m libuv/%s%s" : " * VERSIONS: ReCoal-XMRig/%s libuv/%s%s",
...
static void print_commands(xmrig::Config *config)
{
    if (config->isColors()) {
        ...
        Log::i()->text("ReCoal Mod by ReCoal Dev Team");
    }
    else {
